...
class MainController(rpc: NodeRPCConnection)  {
    ...
    fun createIOU(request: HttpServletRequest): ResponseEntity<String> {
        ...
        return try {
            val signedTx = proxy.startTrackedFlow(::Initiator, iouValue.toDouble(), "USD",otherParty).returnValue.getOrThrow()
    ...
    fun getMyIOUs(): ResponseEntity<List<StateAndRef<IOUState>>>  {
        val myious = proxy.vaultQueryBy<IOUState>().states.filter {
            it.state.data.lender.equals(proxy.nodeInfo().legalIdentities.first())
        }
        myious.get(0).state.notary
